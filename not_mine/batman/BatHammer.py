...
imfilt=str(sys.argv[1])+'_??????.txt'



os.system("ls "+imfilt+" > filterlist.txt")

matches = [line.rstrip('\n') for line in open('filterlist.txt')]
...
##input locations for priors and relative photometry solved data
    ...
    night = np.genfromtxt(matches[x], delimiter='\t',skip_header=1) #read data

    midpoint= np.genfromtxt(matches[x],dtype='str',usecols=(0),comments='earth') #read midpoint (comments=/='#')

    cut=str.find(midpoint[0],'=') ##normalize time to midpoint
    midpoint=midpoint[0][int(cut)+1:]
    night[:,0]=night[:,0]-float(midpoint)
    for y in range (0, len(night)): ##append individual nights together
        dat = np.append(dat,[night[y]],axis=0)

dat = np.delete(dat,[0,1],axis=0) ##clear the inital filler
dat = dat[dat[:,0].argsort()]  #re-sort based on time from midpoints

inpriors = np.genfromtxt('priors.txt', delimiter='\t',skip_header=1) #read data
...
f_true=0.001
t = np.array(dat[:,0]) ##needed to be read correctly by batman
...
#Generate Test Data /  Lookup Table for Test Data###############
#recalculation of light curve with new prameters can be done without reinitializing model
#do this to create all fit data
t0=time.time()


#create test data - this section can be deleted for real data
...
##mcmc##############
#line log likelihm * x + bood function
...
##sample in log(f) and cos(i) (and log(ecc)) space to get a more accurate parameter walking (a more accurate but much more complex parameterization exists from Adam but has not been implimented)
...
tif_ml, per_ml, r_ml, a_ml, cosinc_ml, lnecc_ml, w_ml, lnf_ml = result["x"]
...
sampler.run_mcmc(pos,100000) #run each setup for 500 steps
...
fig = corner.corner(samples[:,[0,2,3,4,5]], labels=["$tif$" , "radius", "$a$", "$i$","$e$"],
                      truths=[tif_true, r_true, a_true, np.cos(inc_true*np.pi/180.), ecc_true]) 
...
fig = corner.corner(samples[:,[2,4,5,6,7]], labels=["radius" , "$i$", "$e$", "$w$","$lnf$"],
                      truths=[r_true, np.cos(inc_true*np.pi/180.), ecc_true, w_true, np.log(f_true)]) 
fig.savefig(sys.argv[1]+"_post_2.png")
fig.clf()


print("Mean acceptance fraction: {0:.3f}".format(np.mean(sampler.acceptance_fraction)))
samples[:, 7] = np.exp(samples[:, 7]) #remake all lnf's into f's
...
tif_mcmc, per_mcmc, r_mcmc, a_mcmc, inc_mcmc, ecc_mcmc, w_mcmc, f_mcmc = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]),  
                             zip(*np.percentile(samples, [16, 50, 84], 
                                                axis=0))) #get the 16,50,and84 percentiles along the unweighted chain for the 2 params 
